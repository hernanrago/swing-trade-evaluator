#!/usr/bin/env python3
"""
Agent: Swing Trade Direction Recommender
Simplified agent that only evaluates trend and recommends direction.
Input: pair (e.g., BTC)
"""
...
def run_agent(pair):
    """
    Runs the agent to evaluate trend and recommend direction.
    Input: pair (e.g., BTC)
    """
    
    system_prompt = """You are a crypto trend analysis agent. Your job is to:

1. Call the evaluate_tf_trend skill with the provided pair
2. Analyze the result (1D and 1W trends)
...
Format your final response as clear, actionable trading information."""

    user_message = f"""
Evaluate the trend for {pair} and recommend a direction (LONG or SHORT).

Analyze the 1D and 1W timeframes and tell me what direction you recommend.
"""
    ...
    print(f"\n[*] Analyzing {pair} trend...", file=sys.stderr)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Evaluate crypto trend and recommend direction")
    ...
    print("\n" + "="*70)
    print("TREND ANALYSIS & DIRECTION RECOMMENDATION")
    print("="*70)
    print(result)
